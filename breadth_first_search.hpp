  ...
  public:
    ...
    auto operator()() const noexcept {
      ...
      while (!std::empty(queue)) {
        const auto& node = queue.front();  // queue.pop()する前にqueue.emplace()しているので、queueがデフォルトで使用するdequeの実装次第ではヤバイかも……。ちなみにRustだと、このコードはダングリング参照の危険性ありということでコンパイル・エラーになります。Rust良いよRust。
